...
export class DynamicQueryBuilderService {
  ...
  private buildWhereConditions<TTable extends PgTable>(
    ...
    if (filterInput.filters) {
      Object.entries(filterInput.filters).forEach(
  ...
  private buildSearchConditions<TTable extends PgTable>(
    ...
    const searchableFields = Object.entries(config.fields)
      ...
      .map(([fieldName, fieldConfig]) => {
        const tableFields = config.table as unknown as Record<string, PgColumn>;
  ...
  private buildFieldCondition<TTable extends PgTable>(
    ...
    const tableFields = table as unknown as Record<string, PgColumn>;
    const column = tableFields[fieldName];
    if (!column) {
      return undefined;
    }
  ...
  private buildOrderByClause<TTable extends PgTable>(
    ...
    const sortField = filterInput.sort.field;
    const sortOrder = filterInput.sort.order;
    ...
    if (!fieldConfig || !fieldConfig.sortable) {
      ...
      if (config.defaultSort) {
        ...
        if (defaultFieldConfig && defaultFieldConfig.sortable) {
          const tableFields = config.table as unknown as Record<
            string,
            PgColumn
          >;
  ...
  applyPagination<T>(query: T, pagination: { page: number; limit: number }): T {
    ...
    const queryWithMethods = query as unknown as {
      limit: (limit: number) => { offset: (offset: number) => T };
    };
    return queryWithMethods.limit(pagination.limit).offset(offset);
  }
  ...
  validateFilterInput(
    ...
    // Validate sort field
    if (filterInput.sort.field) {
      const fieldConfig = config.fields[filterInput.sort.field];
      if (!fieldConfig) {
        errors.push(`Sort field '${filterInput.sort.field}' is not configured`);
      } else if (!fieldConfig.sortable) {
        errors.push(`Field '${filterInput.sort.field}' is not sortable`);
      }
    }
